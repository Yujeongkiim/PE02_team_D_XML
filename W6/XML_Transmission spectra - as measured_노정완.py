...
ax1.scatter(None, None, label=f"R² = {calc_R_squared():4f}")

# Add annotations for current values
annotations = []
for x, y in zip(iv_data['voltage'], iv_data['current']):
    annotation = ax1.annotate(f"{y:.2e}A", xy=(x, y), xytext=(0.5, -20) if x in [-2.0, -1.5, -1, -0.5] else (0.5, 10),
...
for wavelength_sweep in root.iter('WavelengthSweep'):
    ...
    if not wavelength_sweep == list(root.iter('WavelengthSweep'))[-1]:
        ax2.scatter('wavelength', 'measured_transmission', data=wavelength_data, color=color, s=3,
                    label=wavelength_sweep.get('DCBias') + ' V')
    else:
        ax2.scatter('wavelength', 'measured_transmission', data=wavelength_data, color=color, s=3, label=None)
